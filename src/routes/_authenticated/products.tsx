            ...
            <Card key={p.id}>
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex gap-1 justify-end border-t pt-2 -mx-1">
                  ...
                  <Button variant="ghost" size="icon" aria-label={`Edit product ${p.name}`} onClick={() => openEdit(p)}><Pencil className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" aria-label={`Delete product ${p.name}`} onClick={() => { if (confirm(`Delete "${p.name}"?`)) del.mutate(p.id); }}><Trash2 className="h-4 w-4" /></Button>
                </div>
              </CardContent>
            </Card>
          );
      ...
      <Dialog open={quickTxnOpen} onOpenChange={setQuickTxnOpen}>
        <DialogContent className="max-w-[260px] sm:max-w-[280px] rounded-2xl p-4">
          <DialogHeader className="space-y-1">
            <DialogTitle className="text-center text-lg font-semibold">Quick transaction</DialogTitle>
            <p className="text-center text-xs text-muted-foreground">Choose a direction to update stock.</p>
          </DialogHeader>
